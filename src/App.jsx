...
function Btn({ children, onClick, variant = "secondary", disabled, style: extra }) {
  ...
  const base = {
    ...
    padding: "12px 20px", borderRadius: 3, cursor: disabled ? "not-allowed" : "pointer",
    border: "1px solid transparent", WebkitTapHighlightColor: "transparent",
    userSelect: "none", opacity: disabled ? 0.35 : 1,
    transition: "transform 0.12s cubic-bezier(0.22,1,0.36,1), box-shadow 0.18s ease, background 0.18s ease, border-color 0.18s ease, color 0.18s ease",
    transform: pressed ? "translateY(1px) scale(0.985)" : "translateY(0) scale(1)",
    position: "relative", ...extra
  };
  const v = {
    // Primary: a real surface — gradient fill catching light at the top, fine
    // inner highlight, grounded border, and a lift shadow so it sits ABOVE the
    // page. Press collapses the shadow + depresses. This is what stops it
    // reading as a flat translucent rectangle (the generic-AI look).
    primary: { ...base,
      background: hovered
        ? "linear-gradient(180deg, #9168d4 0%, #7C5CBF 100%)"
        : "linear-gradient(180deg, #8460cc 0%, #6e51b0 100%)",
      border: "1px solid #a98fe0",
      color: "#fff",
      boxShadow: pressed
        ? "inset 0 1px 2px rgba(0,0,0,0.3)"
        : `inset 0 1px 0 rgba(255,255,255,0.22), 0 2px 12px ${PURPLE}55, 0 1px 2px rgba(0,0,0,0.4)`,
      textShadow: "0 1px 1px rgba(0,0,0,0.25)"
    },
    secondary: { ...base,
      background: hovered ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.015)",
      border: `1px solid ${hovered ? "rgba(255,255,255,0.28)" : "rgba(255,255,255,0.14)"}`,
      color: hovered ? "#dcdce4" : "#9a9aac",
      boxShadow: pressed ? "inset 0 1px 2px rgba(0,0,0,0.25)" : "inset 0 1px 0 rgba(255,255,255,0.04)"
    },
    ghost: { ...base,
    ...
    danger: { ...base,
      background: hovered ? "rgba(239,83,80,0.12)" : "rgba(239,83,80,0.03)",
      border: `1px solid ${hovered ? "rgba(239,83,80,0.55)" : "rgba(239,83,80,0.22)"}`,
