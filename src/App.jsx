...
function SharedCard({ payload }) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${PURPLE}40`, borderRadius: 20, padding: "36px 32px",
...
// Render the card to a PNG via SVG → canvas (no external deps). Returns a
// promise of a data URL. Uses a foreignObject so the HTML card renders as-is.
async function cardToPng(payload, scale = 2) {
  const W = 600, H = estimateCardHeight(payload);
  // Build an SVG that embeds the card markup as XHTML in a foreignObject.
  const html = buildCardSvgHtml(payload, W, H);
...
function estimateCardHeight(payload) {
  let h = 360; // base: header, decision, score, footer
  if (payload?.v) h += Math.ceil((payload.v.length || 0) / 52) * 22 + 30;
  if (Array.isArray(payload?.f)) h += payload.f.length * 34 + 40;
  h += Math.ceil((payload?.d?.length || 0) / 36) * 30;
  return Math.max(440, h);
}

// Inline-styled XHTML mirroring SharedCard, for the SVG image. Kept self-contained
// (no external CSS/fonts) so the rendered PNG is deterministic.
function buildCardSvgHtml(p, W, H) {
  const col = scoreColor(p?.b ?? 0);
  const esc = s => String(s || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const fails = (Array.isArray(p?.f) ? p.f : []).map((x, i) =>
    `<div style="display:flex;gap:10px;margin-bottom:10px;"><span style="font-size:11px;color:${PURPLE_BRIGHT};font-weight:700;">${String(i + 1).padStart(2, "0")}</span><span style="font-size:13px;color:#e6e6ee;font-weight:600;">${esc(x.m)}<span style="color:#6a6a82;font-weight:400;"> · ${esc(x.r)}</span></span></div>`
  ).join("");
  return `<div xmlns="http://www.w3.org/1999/xhtml" style="width:${W}px;height:${H}px;box-sizing:border-box;background:linear-gradient(160deg,#0d0d16,#14101f);border:1px solid ${PURPLE}66;border-radius:20px;padding:34px 30px;font-family:Arial,Helvetica,sans-serif;">
    <div style="display:flex;align-items:center;gap:9px;margin-bottom:24px;"><span style="color:${PURPLE_BRIGHT};font-size:22px;font-weight:700;">V</span><span style="font-size:18px;color:#fff;">Vestige</span><span style="margin-left:auto;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#6a6a82;">Decision Brittleness</span></div>
    <div style="font-size:11px;letter-spacing:1.2px;text-transform:uppercase;color:#6a6a82;margin-bottom:8px;">The decision</div>
    <div style="font-size:23px;line-height:1.3;color:#fff;margin-bottom:22px;">${esc(p?.d)}</div>
    <div style="display:flex;align-items:baseline;gap:12px;margin-bottom:22px;"><span style="font-size:60px;line-height:1;color:${col};">${p?.b ?? 0}</span><span style="font-size:13px;color:${col};font-weight:600;">${scoreLabel(p?.b ?? 0)} brittleness<br/><span style="font-size:11px;color:#6a6a82;font-weight:400;">out of 100</span></span></div>
    ${p?.v ? `<div style="border-left:2px solid ${PURPLE}88;padding-left:13px;margin-bottom:22px;"><span style="font-size:14px;line-height:1.6;color:#c9c9d6;font-style:italic;">${esc(p.v)}</span></div>` : ""}
    ${fails ? `<div style="font-size:11px;letter-spacing:1.2px;text-transform:uppercase;color:#6a6a82;margin-bottom:12px;">How it could break</div>${fails}` : ""}
    <div style="margin-top:24px;padding-top:14px;border-top:1px solid #ffffff14;text-align:center;font-size:11px;color:#56566a;">Scored by Vestige — decision intelligence</div>
  </div>`;
}
...
function Btn({ children, onClick, variant = "secondary", disabled, style: extra }) {
  ...
  const base = {
    ...
    padding: "12px 22px", borderRadius: 8, cursor: disabled ? "not-allowed" : "pointer",
    border: "none", transition: "all 0.15s", WebkitTapHighlightColor: "transparent",
    userSelect: "none", opacity: disabled ? 0.4 : pressed ? 0.75 : 1,
    transform: pressed ? "scale(0.96)" : "scale(1)", ...extra
  };
  const v = {
    primary:   { ...base, background: hovered ? "linear-gradient(135deg,#8a6acc,#a044f0)" : `linear-gradient(135deg,${PURPLE},#9333ea)`, color: "#fff" },
    secondary: { ...base, background: "transparent", border: `1px solid ${hovered ? "#ffffff22" : "#ffffff12"}`, color: hovered ? "#999" : "#666" },
    ghost:     { ...base, background: "none", border: "none", color: hovered ? "#888" : "#444" },
    danger:    { ...base, background: hovered ? "#1a0808" : "none", border: `1px solid ${hovered ? "#5a2020" : "#2a1010"}`, color: hovered ? "#c33" : "#3a1a1a" },
  };
  return (
    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 12 }}>
        ...
        <button onClick={onSubmit} disabled={!value.trim()} style={{
          background: value.trim() ? `linear-gradient(135deg, ${PURPLE}, ${CYAN}88)` : "rgba(20,20,32,0.8)",
          border: value.trim() ? "1px solid rgba(255,255,255,0.15)" : "1px solid rgba(255,255,255,0.05)",
          color: value.trim() ? "#fff" : "#333",
          fontSize: 14, fontWeight: 600, padding: "12px 26px", borderRadius: 12,
          cursor: value.trim() ? "pointer" : "not-allowed",
          fontFamily: "inherit", WebkitTapHighlightColor: "transparent", minHeight: 46,
          transition: "all 0.2s",
          boxShadow: value.trim() ? `0 4px 24px ${PURPLE}55, inset 0 1px 0 rgba(255,255,255,0.15)` : "none",
          display: "flex", alignItems: "center", gap: 8
        }}
        onMouseEnter={e => { if (value.trim()) { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = `0 6px 28px ${PURPLE}77, inset 0 1px 0 rgba(255,255,255,0.2)`; }}}
        onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = value.trim() ? `0 4px 24px ${PURPLE}55, inset 0 1px 0 rgba(255,255,255,0.15)` : "none"; }}
        >
          Analyse
...
Return as plain text, NOT JSON.`
  ...
  return (
    <div onClick={() => setOpen(o => !o)} style={{
      background: "linear-gradient(135deg, rgba(13,13,22,0.7), rgba(15,12,24,0.5))",
      backdropFilter: "blur(16px)",
      WebkitBackdropFilter: "blur(16px)",
      border: "1px solid rgba(255,255,255,0.05)",
      borderLeft: `2px solid ${col}66`, borderRadius: 16, padding: "20px 22px",
    ...
    onMouseEnter={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(18,16,28,0.8), rgba(20,16,32,0.6))"; e.currentTarget.style.borderLeftColor = col + "99"; }}
    onMouseLeave={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(13,13,22,0.7), rgba(15,12,24,0.5))"; e.currentTarget.style.borderLeftColor = col + "66"; }}
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 9, color: "#252535", letterSpacing: 3, fontWeight: 600, marginBottom: 4 }}>
            {(roleLabels[f.role] || f.role).toUpperCase()}
          </div>
          <div style={{ fontSize: 15, fontWeight: 600, color: "#888" }}>{f.failure_mode}</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: col, background: col + "11", padding: "4px 10px", borderRadius: 6, minWidth: 44, textAlign: "center" }}>{sc}</div>
          <span style={{ fontSize: 11, color: "#2a2a3a", display: "inline-block", transform: open ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>v</span>
        </div>
      </div>
...
function ScoreBar({ brittleness, animate }) {
  ...
  return (
    <div style={{
      background: "linear-gradient(135deg, rgba(13,13,22,0.8), rgba(20,16,32,0.6))",
      backdropFilter: "blur(20px)",
      WebkitBackdropFilter: "blur(20px)",
      ...
      position: "relative",
      overflow: "hidden",
      boxShadow: animate ? `0 0 120px ${col}22, 0 24px 60px rgba(0,0,0,0.5)` : "0 24px 60px rgba(0,0,0,0.3)",
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <div style={{
          fontSize: 10, color: "#3a3a4a", letterSpacing: 5,
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 14, marginBottom: 24 }}>
          <span style={{
            ...
            color: animate ? col : "#1a1a28",
            lineHeight: 0.9, letterSpacing: "-0.04em",
            transition: "color 1s ease",
            animation: animate ? "countUp 0.6s ease" : "none",
            textShadow: animate ? `0 0 40px ${col}55` : "none"
          }}>{brittleness}</span>
          <div style={{ paddingBottom: 14 }}>
            <div style={{ fontSize: 13, color: "#3a3a4a", marginBottom: 4, fontWeight: 500 }}>/ 100</div>
            <div style={{
              fontSize: 14, fontWeight: 700, color: col,
              letterSpacing: 3, textTransform: "uppercase"
            }}>{scoreLabel(brittleness)}</div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            height: "100%", width: animate ? `${brittleness}%` : "0%",
            background: `linear-gradient(90deg, ${PURPLE}, ${col})`,
            borderRadius: 2,
...
function StakesCalibration({ onContinue, onSkip }) {
  ...
  return (
    <div style={{
      background: "linear-gradient(135deg, rgba(13,13,22,0.85), rgba(15,12,24,0.7))",
      backdropFilter: "blur(20px)",
...
function ThreadAnalysisCard({ analysis, onRefresh, loading }) {
  if (loading) {
    return (
      <div style={{
        ...
        borderRadius: 18, padding: "40px 22px", marginBottom: 12,
  ...
  return (
    <div style={{
      ...
      backdropFilter: "blur(20px)",
      border: `1px solid ${CYAN}33`,
      borderRadius: 18, padding: "24px 22px", marginBottom: 12
    }}>
...
function LensSelectorModal({ lenses, entry, onApply, onClose }) {
  ...
  return (
    ...
    >
      <div style={{
        background: "linear-gradient(135deg, rgba(13,13,22,0.95), rgba(15,12,24,0.85))",
        backdropFilter: "blur(20px)",
        border: `1px solid ${PURPLE}33`,
...
function AnatomyCard({ anatomy }) {
  ...
  return (
    <div style={{
      ...
      WebkitBackdropFilter: "blur(20px)",
      border: "1px solid rgba(255,255,255,0.05)",
      borderRadius: 18, padding: "26px 24px", marginBottom: 16
    }}>
      <div style={{ fontSize: 10, color: "#3a3a4a", letterSpacing: 5, fontWeight: 600, marginBottom: 20 }}>THE DECISION</div>
      {rows.map(([label, value]) => {
        const isStrength = label === "Strengths";
        return (
          <div key={label} style={{ display: "flex", gap: 14, marginBottom: 10, flexWrap: "wrap" }}>
            <span style={{ fontSize: 10, color: isStrength ? "#22c55e" : "#252535", fontWeight: 600, letterSpacing: 2, minWidth: 88, paddingTop: 2, flexShrink: 0, opacity: isStrength ? 0.75 : 1 }}>{label.toUpperCase()}</span>
            <span style={{ fontSize: 13, color: isStrength ? "#7a9a82" : "#555", lineHeight: 1.6, flex: 1, minWidth: 180, wordBreak: "break-word" }}>{value}</span>
          </div>
        );
      })}
    </div>
...
function SignalsCard({ signals }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, color: PURPLE_BRIGHT, letterSpacing: 4, fontWeight: 600, marginBottom: 8, opacity: 0.7 }}>WHAT WOULD CHANGE YOUR MIND</div>
      <p style={{ fontSize: 12, color: "#444", marginBottom: 16, lineHeight: 1.6 }}>Watch for these. If any cross their threshold, your assumptions need re-examining.</p>
        ...
        {signals.map((s, i) => (
          <div key={i} style={{
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
              <div style={{
                width: 24, height: 24, borderRadius: 6, background: `${PURPLE}22`,
                display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                color: PURPLE_BRIGHT, fontSize: 11, fontWeight: 700
              }}>{i + 1}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, color: "#bbb", fontWeight: 600, marginBottom: 4 }}>{s.metric}</div>
                <div style={{ fontSize: 12, color: "#666", lineHeight: 1.55 }}>
                  <span style={{ color: PURPLE_BRIGHT, fontWeight: 600 }}>Threshold: </span>{s.threshold}
                </div>
                {s.watches && (
                  <div style={{ fontSize: 10, color: "#444", marginTop: 6, letterSpacing: 1 }}>
                    Tied to: {s.watches}
                  </div>
                )}
              </div>
...
function PostMortemPrompt({ entry, onSubmit, onSkip }) {
  ...
  if (phase === "reveal") {
    return (
      <div style={{
        background: "linear-gradient(135deg, rgba(13,13,22,0.9), rgba(15,12,24,0.75))",
        backdropFilter: "blur(20px)", border: `1px solid ${PURPLE}33`,
        borderRadius: 18, padding: "24px 22px", marginBottom: 20, animation: "fadeUp 0.3s ease"
      }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 120, textAlign: "center", padding: "16px 12px", background: `${CYAN}0a`, border: `1px solid ${CYAN}22`, borderRadius: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 120, textAlign: "center", padding: "16px 12px", background: `${col}0a`, border: `1px solid ${col}33`, borderRadius: 12 }}>
...
function CommitmentBlock({ entry, onCommit, memory = [], onOpen }) {
  ...
  if (committed) {
    ...
    return (
      <div style={{
        background: "linear-gradient(135deg, rgba(13,13,22,0.7), rgba(15,12,24,0.5))",
        backdropFilter: "blur(14px)",
        border: `1px solid ${moveColors[entry.commitment.move]}33`,
        borderLeft: `2px solid ${moveColors[entry.commitment.move]}88`,
        borderRadius: 14, padding: "18px 20px", marginBottom: 12
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ fontSize: 10, color: moveColors[entry.commitment.move], letterSpacing: 3, fontWeight: 600, opacity: 0.9 }}>YOUR COMMITMENT</div>
          <button onClick={() => setEditing(true)} style={{
            background: "none", border: "none", color: "#444", fontSize: 11, cursor: "pointer",
            fontFamily: "inherit", padding: 0
          }}>edit</button>
        </div>
        <div style={{ fontSize: 15, color: "#bbb", fontWeight: 600, marginBottom: entry.commitment.reviewDate ? 6 : 0 }}>
        ...
        {entry.commitment.notes && (
          <p style={{ fontSize: 12, color: "#777", lineHeight: 1.6, margin: "10px 0 0", paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.04)", fontStyle: "italic" }}>{entry.commitment.notes}</p>
        )}
        {entry.commitment.overrodeChallenge && (
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.04)" }}>
            ...
            <span style={{ fontSize: 10, color: entry.commitment.overrodeChallenge === "high" ? "#9a6a6a" : "#9a7a5a", letterSpacing: 0.5 }}>Committed against your own prior evidence</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, color: PURPLE_BRIGHT, letterSpacing: 4, fontWeight: 600, marginBottom: 4, opacity: 0.7 }}>YOUR MOVE</div>
      <p style={{ fontSize: 12, color: "#444", marginBottom: 14 }}>Commit to a path. Vestige tracks what you actually do.</p>
...
function ArchetypeContext({ archetype, context }) {
  ...
  return (
    <div style={{
      background: "linear-gradient(135deg, rgba(94,212,212,0.04), rgba(124,92,191,0.03))",
      ...
      <div style={{
        ...
        padding: "3px 8px", background: `${CYAN}11`, borderRadius: 4,
        flexShrink: 0, marginTop: 2, opacity: 0.85
      }}>{(labels[archetype] || archetype).toUpperCase()}</div>
      <p style={{ fontSize: 12, color: "#666", lineHeight: 1.6, margin: 0, flex: 1 }}>{context}</p>
    </div>
  );
}
...
function Toast({ msg }) {
  ...
  return (
    <div style={{
      ...
      padding: "12px 22px", borderRadius: 12, zIndex: 200,
...
function ReasoningTrace({ memory, result }) {
  ...
  return (
    <div style={{
      background: "linear-gradient(135deg, rgba(13,13,22,0.6), rgba(15,12,24,0.4))",
      border: "1px solid rgba(255,255,255,0.06)",
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={grounding.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 10, color: PURPLE_BRIGHT, letterSpacing: 3, fontWeight: 600, opacity: 0.7 }}>HOW VESTIGE READ THIS</div>
            <div style={{ fontSize: 11.5, color: grounding.color, marginTop: 3, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{grounding.label}</div>
          </div>
        </div>
        <span style={{ fontSize: 11, color: "#444", flexShrink: 0, transform: open ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>▾</span>
      </button>
      {open && (
...
function InheritedWarning({ warnings, memory, onOpen }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 16, padding: "22px 22px", marginBottom: 16
    }}>
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: "linear-gradient(135deg, rgba(13,13,22,0.97), rgba(15,12,24,0.92))",
        backdropFilter: "blur(20px)", border: `1px solid ${PURPLE}33`,
        borderRadius: 18, padding: "26px 24px", maxWidth: 460, width: "100%",
        ...
        {/* Free */}
        <div style={{ border: `1px solid ${plan === "free" ? PURPLE + "55" : "rgba(255,255,255,0.08)"}`, borderRadius: 14, padding: "18px 18px", marginBottom: 12, background: plan === "free" ? `${PURPLE}0d` : "transparent" }}>
        ...
        {/* Pro */}
        <div style={{ border: `1px solid ${plan === "pro" ? "#d4b48355" : CYAN + "44"}`, borderRadius: 14, padding: "18px 18px", marginBottom: 18, background: plan === "pro" ? "rgba(212,180,131,0.06)" : `${CYAN}08` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
...
function Paywall({ memory, usage, onUpgrade, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: "linear-gradient(135deg, rgba(13,13,22,0.97), rgba(20,16,32,0.92))",
        backdropFilter: "blur(20px)", border: `1px solid ${CYAN}33`,
        borderRadius: 18, padding: "28px 24px", maxWidth: 440, width: "100%", animation: "fadeUp 0.25s ease",
        ...
        <div style={{ background: `${CYAN}0a`, border: `1px solid ${CYAN}22`, borderRadius: 12, padding: "14px 16px", marginBottom: 20 }}>
...
function ShareModal({ target, onClose, showToast }) {
  ...
  async function downloadImage() {
    setDl(true);
    try {
      const dataUrl = await cardToPng(payload, 2);
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: "linear-gradient(135deg, rgba(13,13,22,0.98), rgba(20,16,32,0.94))",
        border: `1px solid ${PURPLE}40`, borderRadius: 20, padding: "26px 24px",
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        borderRadius: 20, padding: "30px 26px", maxWidth: 440, width: "100%",
...
function SyncModal({ user, syncState, onClose, onAuthed, showToast }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: "linear-gradient(135deg, rgba(13,13,22,0.97), rgba(15,12,24,0.9))",
        backdropFilter: "blur(20px)", border: `1px solid ${CYAN}33`,
        borderRadius: 18, padding: "26px 24px", maxWidth: 420, width: "100%",
        animation: "fadeUp 0.2s ease"
      }}>
...
export default function App() {
  ...
  const SH = { fontSize: 10, color: PURPLE_BRIGHT, letterSpacing: 4, fontWeight: 600, marginBottom: 14, padding: "0 2px", opacity: 0.7 };
  const iconBtn = {
  ...
  return (
    <div style={{ background: "#08080d", minHeight: "100vh", color: "#fff", fontFamily: "'Inter', system-ui, sans-serif", overscrollBehavior: "none", position: "relative", overflowX: "hidden", touchAction: "pan-y" }}>
      ...
      <nav style={{
        ...
        <div className="nav-cluster" style={{ display: "flex", gap: 8, alignItems: "center", overflowX: "auto", maxWidth: "72vw", flexShrink: 0 }}>
      ...
      {view === "result" && result && (
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.5s ease", position: "relative" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, position: "relative", zIndex: 1, flexWrap: "wrap" }}>
            <span style={{ fontSize: 10, color: PURPLE_BRIGHT, letterSpacing: 4, fontWeight: 600, opacity: 0.7 }}>ANALYSIS COMPLETE</span>
            {result.calibrated && (
          ...
          <div style={{
            ...
            boxShadow: `0 0 60px ${PURPLE}11`
          }}>
            <div style={{ fontSize: 10, color: PURPLE_BRIGHT, letterSpacing: 5, fontWeight: 600, marginBottom: 18, opacity: 0.7 }}>THE VERDICT</div>
            <p style={{
              fontFamily: "'Instrument Serif', Georgia, serif",
              fontSize: 19, color: "#bbb", lineHeight: 1.7,
      ...
      {view === "history" && (
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.4s ease" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 36 }}>
            ...
            {memory.length > 0 && !confirmClear && <Btn variant="danger" onClick={() => setConfirmClear(true)} style={{ fontSize: 12, padding: "8px 14px" }}>Clear all</Btn>}
          </div>
                ...
                <div key={e.id} onClick={() => openDecision(e)}
                  ...
                  style={{
                    background: e.retention === "compress"
                      ...
                      : e.retention === "partial"
                      ? "linear-gradient(135deg, rgba(13,13,22,0.55), rgba(15,12,24,0.4))"
                      : "linear-gradient(135deg, rgba(13,13,22,0.7), rgba(15,12,24,0.5))",
                    backdropFilter: "blur(14px)",
                  ...
                  onMouseLeave={ev => { ev.currentTarget.style.background = "linear-gradient(135deg, rgba(13,13,22,0.7), rgba(15,12,24,0.5))"; ev.currentTarget.style.borderColor = "rgba(255,255,255,0.05)"; ev.currentTarget.style.transform = "translateX(0)"; }}
        ...
        <div {...detailSwipe} style={{ maxWidth: 720, margin: "0 auto", padding: "100px 20px 80px", animation: "fadeUp 0.4s ease" }}>
          ...
          {!selected.full ? (
            <div style={{ textAlign: "center", padding: "60px 20px" }}>
              ...
              {selected.lesson && (
                <div style={{ maxWidth: 420, margin: "24px auto 0", padding: "14px 18px", background: `${CYAN}0a`, border: `1px solid ${CYAN}22`, borderRadius: 12 }}>
            ...
            <>
              ...
              <div style={{
                background: "linear-gradient(135deg, rgba(13,13,22,0.7), rgba(20,16,32,0.5))",
                backdropFilter: "blur(20px)",
                WebkitBackdropFilter: "blur(20px)",
                border: "1px solid rgba(255,255,255,0.06)",
            ...
            <>
              {patterns.summary && (
                <div style={{
                  ...
                  borderRadius: 16, padding: "22px 22px", marginBottom: 20
                }}>
        ...
        <div style={{ maxWidth: 920, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.4s ease" }}>
          ...
          {compareResult && !compareLoading && (
            <>
              ...
              {/* Recommendation */}
              <div style={{
                background: "linear-gradient(135deg, rgba(13,13,22,0.85), rgba(20,16,32,0.7))",
                backdropFilter: "blur(20px)",
                border: "1px solid rgba(255,255,255,0.06)",
                borderLeft: `2px solid ${PURPLE}88`,
              ...
              {openLoops.map(e => {
                ...
                return (
                  <div key={e.id} onClick={() => { openDecision(e); setShowPostMortem(true); }} style={{
                    background: "linear-gradient(135deg, rgba(13,13,22,0.75), rgba(15,12,24,0.55))",
                    backdropFilter: "blur(14px)",
            ...
            <div style={{ marginTop: 40 }}>
              {simulationResults.map((result, i) => (
                <div key={i} style={{
                  ...
                  backdropFilter: "blur(20px)",
                  border: `1px solid ${PURPLE}33`,
                  borderRadius: 18, padding: "24px 22px", marginBottom: 32
                }}>
        ...
        return (
          <div style={{ maxWidth: 720, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.4s ease" }}>
            ...
            {ev.status === "insufficient" && (
              <div style={{ textAlign: "center", padding: "50px 24px", background: "rgba(13,13,22,0.5)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16 }}>
            ...
            {ev.status === "ok" && (
              <>
                <div style={{
                  ...
                  border: `1px solid ${PURPLE}33`, borderRadius: 16, padding: "22px 22px", marginBottom: 24
                }}>
          ...
          <div style={{ maxWidth: 760, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.4s ease" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: 28 }}>
              {[
                ...
                <div key={s.label} style={{ background: "rgba(13,13,22,0.6)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12, padding: "16px 18px" }}>
            ...
            {calibration.status === "insufficient" && memory.filter(e => e.postMortem).length >= 3 && (
              <div style={{ background: "rgba(13,13,22,0.5)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: "16px 20px", marginBottom: 20 }}>
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.4s ease" }}>
          ...
          {lensResults && (
            <div style={{
              background: "linear-gradient(135deg, rgba(94,212,212,0.04), rgba(13,13,22,0.5))",
              border: "1px solid rgba(94,212,212,0.15)",
              borderRadius: 16, padding: "24px 22px", marginTop: 24
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
