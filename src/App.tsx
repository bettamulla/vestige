...
// Haptic feedback (Android / supported devices; silently no-ops elsewhere, incl.
// iOS Safari which doesn't expose the Vibration API). Used sparingly on moments
// that warrant a physical confirmation — analysis landing, committing a decision.
...
// iOS-style edge-back: the page follows the finger as you drag right, reveals a
// soft purple edge glow as you approach the commit threshold, then either eases
// the rest of the way out (and navigates) or springs back smoothly. No blocky
// instant jump — motion tracks the gesture, with spring easing on release.
function useSwipe(onLeft, onRight) {
  const sx = useRef(null);
  const sy = useRef(null);
  const dragging = useRef(false);
  const [dx, setDx] = useState(0);        // live horizontal offset
  const [settling, setSettling] = useState(false); // true while easing back/out

  const THRESHOLD = 90;      // px past which the gesture commits
  const MAX_HINT = 0.45;     // damping so the page lags slightly behind the finger
  ...
  function onTouchMove(e) {
    ...
    const mx = e.touches[0].clientX - sx.current;
    const my = e.touches[0].clientY - sy.current;
    // Only engage for a clearly-horizontal rightward drag; let vertical scroll pass.
    if (Math.abs(my) > Math.abs(mx)) return;
    if (mx > 0 && onRight) {
      // Damped follow: page moves a fraction of finger distance (resistance feel).
      setDx(mx * MAX_HINT + (mx > THRESHOLD ? (mx - THRESHOLD) * 0.15 : 0));
    }
  }
  function onTouchEnd(e) {
    if (!dragging.current) return;
    dragging.current = false;
    if (sx.current === null) { setDx(0); return; }
    const fdx = e.changedTouches[0].clientX - sx.current;
    const fdy = e.changedTouches[0].clientY - sy.current;
    sx.current = null;
    setSettling(true);
    if (Math.abs(fdx) > Math.abs(fdy) && fdx > THRESHOLD && onRight) {
      // Commit: ease the page the rest of the way out, then navigate.
      setDx(window.innerWidth * 0.5);
      setTimeout(() => { onRight(); setDx(0); setSettling(false); }, 180);
    } else if (Math.abs(fdx) > Math.abs(fdy) && fdx < -THRESHOLD && onLeft) {
      onLeft(); setDx(0); setSettling(false);
    } else {
      // Spring back.
      setDx(0);
  ...
  const progress = Math.min(1, Math.abs(dx) / (THRESHOLD * MAX_HINT));
  const bind = {
    onTouchStart, onTouchMove, onTouchEnd,
    style: {
      transform: dx ? `translateX(${dx}px)` : undefined,
      transition: settling ? "transform 0.26s cubic-bezier(0.22,1,0.36,1)" : "none",
      // soft shadow on the leading edge while dragging, intensifying with progress
      boxShadow: dx > 4 ? `-24px 0 48px rgba(0,0,0,${0.25 * progress})` : undefined
    },
    progress
  };
  return bind;
...
// ── Icon action button ──────────────────────────────────────────────────────
// A uniform square control for toolbars where many secondary actions compete for
// limited width (e.g. the detail-view action bar). Every action gets identical
// treatment — equal size, hairline edge, a single accent on hover — plus an
// accessible label that surfaces as a tooltip. `accent` tints the hover state so
// related actions can still read as a family without colour chaos.
function IconBtn({ children, onClick, label, accent = "rgba(255,255,255,0.55)", active, disabled, busy }) {
  const [hovered, setHovered] = useState(false);
  const [pressed, setPressed] = useState(false);
  const showAccent = hovered || active;
  return (
    <button
      ...
      style={{
        ...
        background: active ? `${accent}1f` : (hovered ? "rgba(255,255,255,0.04)" : "transparent"),
        border: `1px solid ${showAccent ? accent : "rgba(255,255,255,0.10)"}`,
        borderRadius: 2, cursor: disabled ? "not-allowed" : (busy ? "wait" : "pointer"),
        color: showAccent ? accent : "#7a7a8c",
        opacity: disabled ? 0.3 : (pressed ? 0.65 : 1),
        WebkitTapHighlightColor: "transparent",
        transition: "background 0.16s ease, border-color 0.16s ease, color 0.16s ease, opacity 0.1s ease",
        padding: 0
      }}
    >{children}</button>
  );
...
function Btn({ children, onClick, variant = "secondary", disabled, style: extra }) {
  ...
  const base = {
    ...
    padding: "8px 16px", borderRadius: 2, cursor: disabled ? "not-allowed" : "pointer",
    border: "1px solid transparent", WebkitTapHighlightColor: "transparent",
    userSelect: "none", opacity: disabled ? 0.3 : 1, lineHeight: 1,
    transition: "background 0.18s ease, border-color 0.18s ease, color 0.18s ease, opacity 0.12s ease",
    opacity: pressed ? 0.7 : (disabled ? 0.3 : 1),
    position: "relative", whiteSpace: "nowrap", ...extra
  };
  const v = {
    // Primary: restrained dark surface with a crisp purple edge. Sleek = line over
    // fill — a precise bordered control, not a glowing candy blob. Hover lifts the
    // edge and faintly warms the fill. No gloss, no glow, no inner highlight.
    primary: { ...base,
      background: hovered ? "rgba(124,92,191,0.16)" : "rgba(124,92,191,0.08)",
      border: `1px solid ${hovered ? PURPLE_BRIGHT : PURPLE}`,
      color: hovered ? "#fff" : "#cdbdf0"
    },
    secondary: { ...base,
      background: "transparent",
      border: `1px solid ${hovered ? "rgba(255,255,255,0.24)" : "rgba(255,255,255,0.10)"}`,
      color: hovered ? "#dcdce4" : "#8a8a9a"
    },
    ghost: { ...base,
      background: "none",
      border: "1px solid transparent",
      color: hovered ? "#bcbccc" : "#6a6a7a"
    },
    danger: { ...base,
      background: "transparent",
      border: `1px solid ${hovered ? "rgba(239,83,80,0.5)" : "rgba(239,83,80,0.2)"}`,
      color: hovered ? "#e88" : "#9a5a5a"
    },
  };
  return (
    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 12 }}>
        ...
        <button onClick={onSubmit} disabled={!value.trim()} style={{
          background: value.trim() ? "rgba(124,92,191,0.10)" : "transparent",
          border: value.trim() ? `1px solid ${PURPLE}` : "1px solid rgba(255,255,255,0.08)",
          color: value.trim() ? "#cdbdf0" : "#333",
          fontFamily: "'JetBrains Mono', ui-monospace, 'SF Mono', Menlo, monospace",
          fontSize: 10.5, fontWeight: 500, letterSpacing: "0.16em", textTransform: "uppercase",
          padding: "9px 18px", borderRadius: 2, lineHeight: 1,
          cursor: value.trim() ? "pointer" : "not-allowed",
          WebkitTapHighlightColor: "transparent",
          transition: "background 0.18s ease, border-color 0.18s ease, color 0.18s ease",
          display: "flex", alignItems: "center", gap: 8
        }}
        onMouseEnter={e => { if (value.trim()) { e.currentTarget.style.background = "rgba(124,92,191,0.16)"; e.currentTarget.style.borderColor = PURPLE_BRIGHT; e.currentTarget.style.color = "#fff"; }}}
        onMouseLeave={e => { if (value.trim()) { e.currentTarget.style.background = "rgba(124,92,191,0.10)"; e.currentTarget.style.borderColor = PURPLE; e.currentTarget.style.color = "#cdbdf0"; }}}
        >
          Analyse
...
Return as plain text, NOT JSON.`
  ...
  return (
    <div onClick={() => setOpen(o => !o)} style={{
      ...
      animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) both", animationDelay: `${i * 0.1}s`,
...
// Count-up: eases a number from 0 → target once `run` is true. requestAnimationFrame
// with an ease-out curve so the score "settles" like a real instrument reading.
...
function ScoreBar({ brittleness, animate }) {
  ...
  const shown = useCountUp(brittleness, animate);
  return (
    <div style={{
      position: "relative",
...
export default function App() {
  ...
  useEffect(() => {
    if (view === "result") setTimeout(() => { setAnimScore(true); haptic([14, 40, 14]); }, 600);
  ...
  return (
    <div style={{ background: "#08080d", minHeight: "100vh", color: "#fff", fontFamily: "'Inter', system-ui, sans-serif", position: "relative", overflowX: "hidden" }}>
      ...
      <style>{`
        ...
        @keyframes histIn { from{ transform: translateY(14px) } to{ transform: translateY(0) } }
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0 }
        html { -webkit-text-size-adjust:100%; text-size-adjust:100%; overflow-x:hidden; width:100%; }
        html, body { touch-action: pan-y; overscroll-behavior:none; overscroll-behavior-x:none; }
        body { -webkit-overflow-scrolling:touch; overflow-x:hidden; width:100%; max-width:100%; position:relative; }
        /* Native feel: no long-press callout on controls, but keep content selectable */
        button, a, svg, .vestige-mark { -webkit-touch-callout:none; -webkit-user-select:none; user-select:none; }
      ...
      <nav style={{
        ...
        <div className="nav-cluster" style={{ display: "flex", gap: 8, alignItems: "center", overflowX: "auto", flex: "1 1 auto", minWidth: 0, justifyContent: "flex-end", marginLeft: 12 }}>
          ...
          {plan === "free" ? (
            ...
            <button onClick={() => setShowPlans(true)} title="Vestige Pro" style={{
              background: "rgba(124,92,191,0.08)", border: `1px solid ${PURPLE}`,
              color: PURPLE_BRIGHT, fontSize: 10, fontWeight: 500, letterSpacing: "0.18em",
            ...
            onMouseEnter={e => { e.currentTarget.style.background = "rgba(124,92,191,0.16)"; e.currentTarget.style.borderColor = PURPLE_BRIGHT; e.currentTarget.style.color = "#fff"; }}
            onMouseLeave={e => { e.currentTarget.style.background = "rgba(124,92,191,0.08)"; e.currentTarget.style.borderColor = PURPLE; e.currentTarget.style.color = PURPLE_BRIGHT; }}
            >
              <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M12 2l2.4 7.4H22l-6 4.6 2.3 7.4-6.3-4.6L5.7 21l2.3-7.4-6-4.6h7.6z"/></svg>
              Pro
            </button>
          )}
        </div>
        {view !== "landing" && <Btn variant="primary" onClick={() => goAnalyse()} style={{ fontSize: 12, padding: "8px 16px", minHeight: 36, flexShrink: 0, marginLeft: 8 }}>New</Btn>}
      </nav>
      ...
      {/* LANDING */}
      {view === "landing" && (
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)", paddingTop: "calc(96px + var(--safe-top))", touchAction: "pan-y", overflowX: "hidden" }}>
      ...
      {/* ANALYSE */}
      {view === "analyse" && (
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "124px 24px 80px", animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)" }}>
      ...
      {/* RESULT */}
      {view === "result" && result && (
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.55s cubic-bezier(0.22,1,0.36,1)", position: "relative" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 7, alignItems: "center", overflowX: "auto", justifyContent: "flex-end", minWidth: 0 }}>
              <IconBtn onClick={() => { setInput(result.anatomy?.decision || ""); setView("analyse"); }} label="Edit and re-run" accent="rgba(255,255,255,0.7)">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
              </IconBtn>
              <IconBtn onClick={() => exportResult(result)} label="Export report" accent="rgba(255,255,255,0.7)">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
              </IconBtn>
              <IconBtn onClick={() => setShareTarget(result)} label="Share" accent={PURPLE_BRIGHT}>
              ...
              {result.entryId && (
                <IconBtn onClick={() => followOnFrom({ id: result.entryId, decision: result.anatomy?.decision })} label="Follow-on decision" accent={CYAN}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="12" x2="21" y2="12"/><polyline points="14 5 21 12 14 19"/></svg>
                </IconBtn>
              )}
              <IconBtn onClick={() => setView("history")} label="View history" accent="rgba(255,255,255,0.7)">
      ...
      {/* HISTORY */}
      {view === "history" && (
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[...memory].reverse().map((e, idx) => (
                <div key={e.id} onClick={() => openDecision(e)}
                  ...
                  style={{
                    animation: `histIn 0.5s cubic-bezier(0.22,1,0.36,1) both`,
      ...
      {view === "detail" && selected && (
        <div
          ...
          style={{ maxWidth: 720, margin: "0 auto", padding: "100px 20px 80px", animation: detailSwipe.style.transform ? "none" : "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)", position: "relative", ...detailSwipe.style }}
        >
          {/* Swipe-back edge hint: a soft purple bar on the left that brightens as
              you approach the commit threshold — tells you the gesture is live. */}
          {detailSwipe.progress > 0.02 && (
            <div style={{
              position: "fixed", top: 0, left: 0, bottom: 0, width: 3, zIndex: 50,
              background: `linear-gradient(180deg, transparent, ${PURPLE_BRIGHT}, transparent)`,
              opacity: detailSwipe.progress, pointerEvents: "none",
              boxShadow: `0 0 ${12 * detailSwipe.progress}px ${PURPLE_BRIGHT}`
            }} />
          )}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, gap: 12 }}>
            <button onClick={() => setView("history")} style={{
              background: "transparent", border: "1px solid rgba(255,255,255,0.10)",
              color: "#8a8a9a", fontSize: 10.5, fontWeight: 500, letterSpacing: "0.14em",
              textTransform: "uppercase", lineHeight: 1,
              padding: "0 14px", height: 38, borderRadius: 2, cursor: "pointer",
              fontFamily: "'JetBrains Mono', ui-monospace, 'SF Mono', Menlo, monospace",
              WebkitTapHighlightColor: "transparent", flexShrink: 0,
              display: "flex", alignItems: "center", gap: 7,
              transition: "background 0.18s ease, border-color 0.18s ease, color 0.18s ease"
            }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.24)"; e.currentTarget.style.color = "#dcdce4"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.10)"; e.currentTarget.style.color = "#8a8a9a"; }}
            >
            ...
            <div style={{ display: "flex", gap: 7, alignItems: "center", overflowX: "auto", justifyContent: "flex-end", minWidth: 0 }}>
              {selected.full && selected.thread && selected.thread.filter(m => m.type === "message" || m.type === "vestige_reply").length >= 2 && (
                <IconBtn onClick={() => fetchThreadAnalysis(selected)} busy={threadLoading} disabled={threadLoading}
                  label="Analyse thread" accent={PURPLE_BRIGHT}
                  active={threadAnalysis && threadAnalysisEntryId === selected.id}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                </IconBtn>
              )}
              {selected.full && lenses.length > 0 && (
                <IconBtn onClick={() => setLensApplicationTarget(selected)} label="Apply lens" accent={CYAN}>
              ...
              {selected.full && (
                <IconBtn onClick={() => followOnFrom(selected)} label="Follow-on decision" accent={CYAN}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="12" x2="21" y2="12"/><polyline points="14 5 21 12 14 19"/></svg>
                </IconBtn>
              )}
              {selected.full && !selected.postMortem && !showPostMortem && (
                <IconBtn onClick={() => setShowPostMortem(true)} label="Post-mortem" accent={PURPLE_BRIGHT}>
              ...
              {selected.full && (
                <IconBtn onClick={() => exportResult(selected.full)} label="Export report" accent="rgba(255,255,255,0.7)">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                </IconBtn>
              )}
              {selected.full && (
                <IconBtn onClick={() => setShareTarget({ ...selected.full, timestamp: selected.timestamp })} label="Share" accent="rgba(255,255,255,0.7)">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.6" y1="13.5" x2="15.4" y2="17.5"/><line x1="15.4" y1="6.5" x2="8.6" y2="10.5"/></svg>
                </IconBtn>
              )}
            </div>
      ...
      {/* PATTERNS */}
      {view === "patterns" && (
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)" }}>
      ...
      {/* COMPARE */}
      {view === "compare" && (
        <div style={{ maxWidth: 920, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)" }}>
          <div style={{ marginBottom: 28 }}>
      ...
      {view === "landscape" && (() => {
        ...
        return (
          <div style={{ maxWidth: 760, margin: "0 auto", padding: "calc(104px + var(--safe-top)) 20px calc(80px + var(--safe-bottom))", animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)" }}>
            <Btn variant="ghost" onClick={() => setView("history")} style={{ padding: "8px 0", fontSize: 13, marginBottom: 16 }}>← Back to History</Btn>
